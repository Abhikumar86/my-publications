...
ex_data$journal %>% table() %>% as.data.frame() %>% 
	...
	wordcloud2::wordcloud2(size = 0.2, minRotation = 0, maxRotation = 0, 
				     rotateRatio = 1, color = terrain.colors(10))
```
...
```{r stype22, fig.cap="Types of plant ecological studies conducted in the Siwalik region"}
ex_data <- read.csv("./data/siwalik-ecology/extracted_data.csv")
s_type <- table(ex_data$study_type) %>% as.data.frame()
## Compute percentages
s_type$fraction <- s_type$Freq / sum(s_type$Freq)
## Compute the cumulative percentages (top of each rectangle)
s_type$ymax <- cumsum(s_type$fraction)
## Compute the bottom of each rectangle
s_type$ymin <- c(0, head(s_type$ymax, n = -1))
## Compute label position
s_type$labelPosition <- (s_type$ymax + s_type$ymin) / 2
## Compute a good label
s_type$label <- paste0(s_type$Var1, "\n (n = ", s_type$Freq, ")")
## Make the plot
ggplot(data = s_type, 
	 aes(ymax = ymax, ymin = ymin, xmax = 4, xmin = 3, fill = Var1)) +
	geom_rect(color = "white", lwd = 1) +
	geom_text(x = 2.15, aes(y = labelPosition, label = label, color = Var1), 
		    size=4.5) +
...
```{r temp23, fig.cap="Annual frequency of scientific publications"}
ex_data <- read.csv("./data/siwalik-ecology/extracted_data.csv")
...
ggplot(data = temp_dat, aes(x = year, y = Freq)) + 
	...
	geom_smooth(color = "red", se = FALSE, span = 1, lwd = 1.2) +
...
```{r map21, fig.cap="Spation distribution of the study sites (reported or estimated from publications)"}
...
```{r}
ex_data <- read.csv("./data/siwalik-ecology/extracted_data.csv")
geo_dat <- table(ex_data$location) %>% as.data.frame()
...
```{r fig.cap="Breakdown of selected studies by study type, location and domain of ecology", out.width="100%"}
ext_dat <- read.csv("./data/siwalik-ecology/extracted_data.csv") %>%
	dplyr::select(study_type, location, domain) %>%
	gather_set_data(1:3)
ext_dat$x <- factor(ext_dat$x, 
			  labels = c("study_type" = "Study type", "location" = "Location", "domain" = "Domain"),
			  levels=c("study_type", "location", "domain"))

ggplot(ext_dat, aes(x, id = id, split = y, value = 1))  +
	geom_parallel_sets(aes(fill = study_type), alpha = 0.3, axis.width = 0.12, show.legend = FALSE) +
	geom_parallel_sets_axes(axis.width = 0.1, fill = "grey80") +
