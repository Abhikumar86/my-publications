...
ggplot(ext_dat, aes(x, id = id, split = y, value = 1))  +
	...
	geom_parallel_sets_labels(color = "black", angle = 0, size = 4.5) +
...
```{r temp23, fig.cap="Annual frequency of scientific publications. The trend line is fitted using 80% LOESS (y ~ x) smoothening. Inset shows the decadal frequency of publications"}
library(dplyr)
library(ggplot2)
...
df <- temp_dat %>% mutate(decade = 
			  	ifelse(year<=1990, "1980s", 
			  		 ifelse(year>=1991 & year<=2000, "1990s", 
			  		 	 ifelse(year>=2001 & year<=2010, "2000s", 
			  		 	 	 ifelse(year>=2011 & year<=2020, "2010s", "2021")
			  		 	 	 )
			  		 	 )
			  		 )
				  )
...
df$decade <- factor(df$decade,
			  levels = c("1980s",  "1990s", "2000s", "2010s", "2021"))
...
p1 <- ggplot(data = df, aes(x = total_articles, y = decade)) + 
	...
	geom_text(aes(label = paste("n=", total_articles)), color = "#69b3a2", size = 4, 
...
ggplot(data = temp_dat, aes(x = year, y = Freq)) + 
	...
	annotation_custom(ggplotGrob(p1), xmin = 1988, xmax = 2005, 
                       ymin = 2.5, ymax = 6)
```
