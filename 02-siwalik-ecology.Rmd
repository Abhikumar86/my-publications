...
#write.csv(M, "./data/siwalik-ecology/bibdata.csv")
```
...
```{r 2-citation, fig.height=6, fig.width=8, fig.cap="Most cited articles (with at least 25 citations) from idenitfied peer-reviewed publications"}
library(ggplot2)
results <- bibliometrix::biblioAnalysis(M, sep = ";")
df <- results$MostCitedPapers[1:9,]
names(df)[1] <- "Paper"
ggplot(data = df, aes(x = TC, y = reorder(Paper, TC))) +
...
```{r 2-keyocc, fig.height=8, fig.width=8, fig.cap="Co-occurrence network of indexed keywords (n = 25) from Scopus. The size of each vertex is proportional to its degree (i.e. the frequency of keywords)"}
M <- read.csv("./data/siwalik-ecology/bibdata.csv")
...
```{r 2-authcol, fig.height=8, fig.width=8, fig.cap="Collaboration network of authors (n = 25) from the selected studies. The size of each vertex is proportional to its degree (i.e. the frequency of authors appearance)"}
M <- read.csv("./data/siwalik-ecology/bibdata.csv")
NetMatrix <- bibliometrix::biblioNetwork(M, analysis = "collaboration", network = "authors", 
				   sep = ";")
net <- bibliometrix::networkPlot(NetMatrix,  normalize = NULL, n = 25, weighted = NULL, 
...
#st_write(ind1.clipped, "./data/siwalik-ecology/ind1_clipped.gpkg")
ind1.clipped <- st_read("./data/siwalik-ecology/ind1_clipped.gpkg")

ex_data <- read.csv("./data/siwalik-ecology/extracted_data.csv")
loc_lab <- ex_data$location %>% table() %>% as.data.frame()
rownames(loc_lab) <- loc_lab$.
ind1.clipped$label <- factor(ind1.clipped$stname, 
				     labels = c(
				     	"CHANDIGARH" = paste0("CH", " (", loc_lab["Chandigarh", 2], ")"),
				     	"DELHI" = NA,
				     	"HARYANA" = paste0("HR", " (", loc_lab["Haryana", 2], ")"),
				     	"HIMACHAL PRADESH" = paste0("HP", " (", loc_lab["Himachal Pradesh", 2], ")"),
				     	"JAMMU & KASHMIR" =  paste0("JK", " (", loc_lab["Jammu and Kashmir", 2], ")"),
				     	"LADAKH" = NA, "PUNJAB" = NA, "RAJASTHAN" = NA,
				     	"UTTAR PRADESH" = NA,
				     	"UTTARAKHAND" = paste0("UK", " (", loc_lab["Uttarakhand", 2], ")"))
				     )
...
```{r 2-map, out.width="100%", fig.cap="Geographic distribution of the study sites (reported or estimated from publications) across the study region (JK = Jammu and Kashmir, HP = Himachal Pradesh, CH = Chandigarh, HR = Haryana, UK = Uttarakhand). The number of studies per region is indicated in parenthesis"}
			  	...
			  	ifelse(year<=1989, "'80s", 
			  		 ifelse(year>=1990 & year<=1999, "'90s", 
			  		 	 ifelse(year>=2000 & year<=2009, "'00s", 
			  		 	 	 ifelse(year>=2010 & year<=2019, "'10s", "'20s")
			  		 	 	 )
			  		 	 )
			  		 )
				  )
...
p1 <- ggplot(data = df, aes(x = total_articles, y = decade)) + 
	...
	geom_text(aes(label = paste0("n=", total_articles)), color = "#69b3a2", 
		    size = 4, vjust = 0.5, hjust = -0.25) +
...
ex_data$study_type1 <- factor(ex_data$study_type, 
			     labels = c("experimental" = paste0("EX", " (", study_lab[1,2], ")"), 
			     	     "modelling" = paste0("MO", " (", study_lab[2,2], ")"),
			     	     "observational" = paste0("OB", " (", study_lab[3,2], ")")))
